    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            # use the old position to get the updated the distribution of the new position
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items(): # go thorugh all the (new position, new probability)
                # new belief[newPos] =  accumulation of (new probability[newPos] * previos belief) 
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = [] # create a list to store particles
        count = 0
        while count < self.numParticles:
            for position in self.legalPositions: # scan all the legal particles' positions
                self.particles.append(position) # append this legal position into particles list
                count += 1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: # a ghost is eaten by Pacman
            self.particles = [self.getJailPosition()] # the ghost appears in its prison cell
        else:
            allPossible = util.Counter() 
            oldBelief = self.getBeliefDistribution()
            for loc in self.legalPositions:
                dist = util.manhattanDistance(loc, pacmanPosition)
                allPossible[loc] += emissionModel[dist] * oldBelief[loc] # calculate the new belief
    
            if any(allPossible.values()): # if there is at least belief which is more than 0
                tmpParticles = []
                for i in range(0, self.numParticles):
                    tmpParticles.append(util.sample(allPossible)) # recreate samples from the previous distribution
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        weight with each position) is incorrect and may produce errors.
        """
        # permutations of legal ghost positions
        ghostsPositions = list(itertools.product(self.legalPositions, repeat = 2))     
        random.shuffle(ghostsPositions) # put ghost positions in random order
    ...
    def observeState(self, gameState):
        ...
        for index, particle in enumerate(self.particles):
            partial = 1.0
            for i in range(self.numGhosts):
                if noisyDistances[i] != None: # a ghost is NOT captured by Pacman
                    distance = util.manhattanDistance(particle[i], pacmanPosition)
                    partial *= emissionModels[i][distance]
                else: # a ghost is captured
                    particle = self.getParticleWithGhostInJail(particle, i)
            allPossible[particle] += partial # accumulate for the same particle 
        
        allPossible.normalize()
        
        if any(allPossible.values()): # if there is at least one belief which is not 0    
            tmpPaticels = []
            for i in range(0, self.numParticles):
                tmpPaticels.append(util.sample(allPossible))
            self.particles = tmpPaticels
        else: # if all the beliefs are 0, need to initialize particles
            self.initializeParticles()
            for index, particle in enumerate(self.particles):
                for i in range(self.numGhosts):
                    if noisyDistances[i] == None: # the ghost is captured
                        particle = self.getParticleWithGhostInJail(particle, i)
    ...
    def getBeliefDistribution(self):
        # the same with the one in ParticleFilter class
        distribution = util.Counter()
        for particle in self.particles:
            distribution[particle] += 1
        distribution.normalize()
